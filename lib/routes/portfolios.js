...
router
    ...
    .put('/sell', bodyParser, (req, res, next) => {
        ...
        Portfolio
            .findOne({userId})
            .then(portfolio => {
                // don't use `in` unless you what to search up the prototypes
                if(!(portfolio.stocks.hasOwnProperty(stock))) throw {code: 403, error: 'You do not currently own any shares of this stock.'};
                if(shares > portfolio.stocks[stock]) throw {code: 403, error: 'You cannot sell more shares than are in your portfolio.'};
                userPortfolio = portfolio;

                // why get all the stocks?
                stockTickers = Object.keys(portfolio.stocks);
                return yahooFinance.snapshot({
                    symbols: stockTickers, // symbols: [stock]?
                    fields: fieldValue
                });
            })
            .then(snapshotArr => {
                if (!snapshotArr[0].ask) throw {code: 503, error: 'API request returning null for ask value'};
                // see yahooApi.js for moving logic into model. 
                // this is duplicative,
                // though not sure why you need to update the other stocks...
                snapshotArr.forEach(snapshot => {
                    stockValueMap[snapshot.symbol] = snapshot.ask;
                });
                let stockValue = 0;
